...
class ControllerTest extends ApplicationTestCase
{
	...
	public function test__construct($test, $check)
	{
		...
		$controller = $this->getMockBuilder('\\FOF40\\Tests\\Stubs\\Controller\\ControllerStub')
			->setMethods(['registerDefaultTask', 'setModelName', 'setViewName'])
			->setConstructorArgs([])
			->setMockClassName('')
	...
	public function testExecuteException()
	{
		...
		ReflectionHelper::setValue($controller, 'taskMap', []);
	...
	public function testDisplay($test, $check)
	{
		...
		$view = $this->getMockBuilder('\\FOF40\\Tests\\Stubs\\View\\ViewStub')
			->setMethods(['setDefaultModel', 'setLayout', 'display'])
	...
	{
		$controller = new ControllerStub(new TestContainer([
			'componentName' => 'com_eastwood',
		]));
	...
	{
		$model      = new ModelStub(new TestContainer([
			'componentName' => 'com_eastwood',
		]));
		$controller = new ControllerStub(new TestContainer([
			'componentName' => 'com_eastwood',
	...
	{
		$view       = new ViewStub(new TestContainer([
			'componentName' => 'com_eastwood',
		]));
		$controller = new ControllerStub(new TestContainer([
			'componentName' => 'com_eastwood',
	...
	public function testGetTasks()
	{
		...
		ReflectionHelper::setValue($controller, 'methods', ['foobar']);

		$tasks = $controller->getTasks();

		$this->assertEquals(['foobar'], $tasks, 'Controller::getTasks failed to return the internal tasks');
	}
	...
	public function testUnregisterTask()
	{
		...
		ReflectionHelper::setValue($controller, 'taskMap', ['foo' => 'bar']);
